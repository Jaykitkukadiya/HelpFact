...
def deshbordcpy(request):
    if str(request.user) != "AnonymousUser" :
        ...
        return render(request, "deshbordcpy.html", {'pending_tasks': pending_tasks, 'completed_task_objs_completed': completed_task_objs_completed , "completed_task_objs_expired" : completed_task_objs_expired , "completed_task_objs_cancelled" : completed_task_objs_cancelled} )
    else:
        return redirect('/login/')
